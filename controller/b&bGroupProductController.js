...
export const mapProductToBandBGroup = async (req, res) => {
  try {
    ...
    if (!product_id || !bnb_group_id) {
      return res
        .status(400)
        .json({ error: "product_id and bnb_group_id are required." });
    ...
    res
      .status(201)
      .json({ message: "Product mapped to B&B Group successfully." });
  } catch (err) {
...
export const bulkMapByNames = async (req, res) => {
  try {
    ...
    if (!bnb_group_name || !product_names || !Array.isArray(product_names)) {
      return res
        .status(400)
        .json({ error: "bnb_group_name and product_names[] are required." });
